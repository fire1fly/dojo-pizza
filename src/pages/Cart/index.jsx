...
import { setEmptyCart, removePizzaItem, plusItemCart, minusItemCart } from '../../redux/actions/cart';
...
export default function Cart() {
  ...
  const addedPizzas = Object.keys(items).map(pizzaId => {
    return items[pizzaId].items[0];
  });
  ...
  const onRemoveCartItem = useCallback(pizzaId => {
    if (window.confirm("Вы уверены, что хотите удалить пиццу")) {
      dispatch(removePizzaItem(pizzaId));
    }
  }, []);

  const onPlusItemCart = useCallback((id) => {
    dispatch(plusItemCart(id));
  }, []);

  const onMinusItemCart = useCallback((id) => {
        ...
        <div className="content__items">
          {
            addedPizzas.map(pizza => {
              const pizzaId = `${pizza.id}_${pizza.size}_${pizza.type}`
              return <CartItem 
                key={pizzaId}
                id={pizzaId}
                name={pizza.name}
                imageUrl={pizza.imageUrl} 
                type={typesNames[pizza.type]} 
                size={pizza.size} 
                totalPrice={items[pizzaId].totalPrice}
                totalCount={items[pizzaId].items.length}
