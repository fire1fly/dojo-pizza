...
export default function CartItem({id, name, imageUrl, type, size, totalPrice, totalCount, onRemoveCartItem, onPlusItemCart, onMinusItemCart}) {

  const hanldeRemovingCartItem = React.useCallback(() => {
    onRemoveCartItem(id);
  });
  const hanldePlus = React.useCallback(() => {
    onPlusItemCart(id);
  });
  const hanldeMinus = React.useCallback(() => {
    onMinusItemCart(id);
  });

  const minusIsDisabled = totalCount === 1 ? true : false;
  ...
  return (
    <div className="cart__item">
      ...
      <div className="cart__item-remove">
        <Button
          onClick={hanldeRemovingCartItem}
